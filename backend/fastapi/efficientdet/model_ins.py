...
class ModelInspector(object):
  ...
  def saved_model_inference(self, image_path_pattern, output_dir, **kwargs):
    ...
    category_index = {k:label_map[k] for k in label_map}  
    ...
    for i in range(num_batches):
      ...
      for j in range(size_before_pad):
        img, classes = driver.visualize(raw_images[j], detections_bs[j], **kwargs)
        img_id = str(i * batch_size + j)
        output_image_path = os.path.join(output_dir, img_id + '.jpg')
        Image.fromarray(img).save(output_image_path)
        print('writing file to %s' % output_image_path)
      #classes = driver.visualize(raw_images[j], detections_bs[j], **kwargs)
      categories = []
      cate_labels = []
      for i in range(len(classes)):
              if i in classes:
                if i not in categories:
                  categories.append(i)
                  cate_labels.append(category_index[i])
                  #category_index[1]
      #print(cate_labels)
      #print(type(cate_labels))
    return cate_labels
  ...
  def run_model(self, runmode, **kwargs):
    ...
    if runmode == 'dry':
      ...
      if runmode == 'saved_model':
        ...
        category_labels = self.saved_model_inference(kwargs['input_image'],
                                   kwargs['output_image_dir'], **config_dict)
